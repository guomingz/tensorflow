...
#
# This script will detect the installed verion of gcc, calculate the bazel
# build flags based on the --disable-mkl and --platform parameters, and
# then write the build options to the --bazelrc-file bazelrc file in the
...
NEHALEM_CPU_INSTRUCTIONS = ["MMX", "SSE", "SSE2", "SSE3", "SSSE3", "SSE4.1",
                            "SSE4.2", "POPCNT"]
...
HASWELL_CPU_INSTRUCTIONS.extend(["FSGSBASE", "RDRND", "FMA",
                                 "BMI", "BMI2", "F16C",
                                 "MOVBE", "AVX2"])

SKYLAKE_CPU_INSTRUCTIONS = HASWELL_CPU_INSTRUCTIONS[:]
SKYLAKE_CPU_INSTRUCTIONS.extend(["PKU", "RDSEED", "ADCX", "PREFETCHW", "CLFLUSHOPT",
                                 "XSAVEC", "XSAVES", "AVX512F", "CLWB",
                                 "AVX512VL", "AVX512BW", "AVX512DQ", "AVX512CD"])

ICELAKE_CPU_INSTRUCTIONS = SKYLAKE_CPU_INSTRUCTIONS[:]
ICELAKE_CPU_INSTRUCTIONS.extend(["AVX512VBMI", "AVX512IFMA", "SHA", "CLWB",
                                 "UMIP", "RDPID", "GFNI", "AVX512VBMI2", "AVX512VPOPCNTDQ",
                                 "AVX512BITALG", "AVX512VNNI", "VPCLMULQDQ", "VAES"])

BASIC_BUILD_OPTS = ["--cxxopt=-D_GLIBCXX_USE_CXX11_ABI=0", "--copt=-O3"]

SECURE_BUILD_OPTS = ["--copt=-Wformat",
...
class BuildEnvSetter:
        ...
        try:
            ...
            if gcc_info[0] < min_gcc_major_version:
                print("Your MAJOR version of GCC is too old: {}; "
                      "it must be at least {}.{}"
                      .format(gcc_info[0], min_gcc_major_version, min_gcc_minor_version))
                return False
                ...
                if gcc_info[1] < min_gcc_minor_version:
                    print("Your MINOR version of GCC is too old: {}; "
                          "it must be at least {}.{}"
                          .format(gcc_info[1], min_gcc_major_version, min_gcc_minor_version))
                    return False
                return True
            else:
    ...
    def parse_args(self):
        arg_parser = ArgumentParser(description="Parse the arguments for the "
                                                "TensorFlow build environment setter")
        arg_parser.add_argument('--disable-mkl', dest="disable_mkl",
                                help="Turn off MKL. By default the compiler flag "
                                     "--config=mkl is enabled.", action="store_true")
        arg_parser.add_argument('--disable-v2', dest="disable_v2",
                                help="Don't build TensorFlow v2. By default the compiler "
                                     " flag --config=v2 is enabled.", action="store_true")
        arg_parser.add_argument('-s', '--secure-build', dest="secure_build",
                                help="Enable secure build flags.", action="store_true")
        arg_parser.add_argument('-p', '--platform', choices=self.PLATFORMS.keys(),
                                help="The target platform.", dest="target_platform",
                                default=self.default_platform_)
        arg_parser.add_argument('-f', '--bazelrc-file', dest="bazelrc_file",
                                help="The full path to the bazelrc file into which the build "
                                     "command will be written. The path will be relative to the "
                                     "container environment.", required=True)
    ...
    def validate_args(self):
        if os.path.exists(self.args.bazelrc_file):
            if os.path.isfile(self.args.bazelrc_file):
                ...
                raise ValueError("{} is not a valid file name".format(self.args.bazelrc_file))
        return True
